...
class DataFile {
public:
    DataFile(string filename) : filename_(filename), fd_(-1), size_(0) {}
...
class IndexFile {
public:
    IndexFile(string filename) : filename_(filename), fd_(-1), size_(0) {}
...
class Database {
public:
    Database(string name) : name_(name), data_file_(name + "/" + data_folder + "/" + name + data_ext), index_file_(name + "/" + index_folder + "/" + name + index_ext) {
    ...
    bool Exists() {
        struct stat buffer;
        return stat((data_folder + "/" + name_).c_str(), &buffer) == 0;
    }
    ...
    bool QueryData(const Window& window, vector<Data>& results) {
        results.clear();
        IndexRecord record;
        long index_offset = 0;
        while (index_file_.Read(index_offset, record)) {
    ...
    bool QueryData_no_index(const Window& window, vector<Data>& results) {
        results.clear();
        long data_offset = 0;
        Data data;
...
class DataProcessor {
public:
    DataProcessor(string data_filename, string index_filename) : data_file_(data_filename), index_file_(index_filename) {}
    ...
    bool Interpolate() {
        ...
        DataFile new_data_file("interpolated_data.bin");
...
int main() {
    ...
    vector<Data> results;
    if (db.QueryData(window, results)) {
    ...
    // Query the data within a window and without index
      if (db.QueryData_no_index(window, results)) {
        cout << "Found " << results.size() << " results:" << endl;
        for (const auto& data : results) {
            cout << "Data: " << data.x << ", " << data.y << ", " << data.z << ", " << data.w << endl;
        }
